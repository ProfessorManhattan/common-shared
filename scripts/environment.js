    ...
    if (fs.existsSync(element)) {
      fs.lstat(element, (err, stats) => {
        if (!stats.isSymbolicLink()) {
          signale.error('The ' + element + ' target in your project root is not a symbolic link. If you plan on using this feature then you should store any folders/files you wish to be considered part of an environment in the environments/{{ environment_name }}/ folder. You can then use this script to handle creating the symbolic links for you. We are skipping the creation of the symlink to environments/' + env + '/' + element + ' because there is a non-symbolic link with the same name in the root of the project.');
          flawless = false;
        } else {
